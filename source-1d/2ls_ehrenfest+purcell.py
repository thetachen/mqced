...
def execute(param_EM,param_TLS,ShowAnimation=False):
    ...
    for it in range(len(times)):
        ...
        Z_save,Ex_save,Ey_save,Bx_save,By_save = EMP.saveFarField(times[it],Tmax,EMP.param.Zb0)
        Zs.append(Z_save)
        Exs.append(Ex_save)
        Eys.append(Ey_save)
        Bxs.append(Bx_save)
        ...
        U_save += (Ex_save**2 + By_save**2)*EMP.param.dZ
        Uemf = EMP.getPartialEnergy(Zmin=0.0,Zmax=EMP.param.Zb0) + U_save
        Uele = TLSP.getEnergy()
        Ut[0,it] = Uele
        Ut[1,it] = Uemf
        ...
        if it%AveragePeriod==0 and ShowAnimation:
            ...
            plt.sca(ax[3])
            plt.cla()
            K_extract=-np.log(np.abs(rhot[n,n,:it])/np.abs(param_TLS.C0[1,0])**2)/times[:it]
            ax[3].plot(times[:it]*AU.fs,K_extract,lw=2,label='Kextract')
            ax[3].axhline(y=KFGR, color='k', linestyle='--', lw=2)
            ...
            #ax[4].set_xlabel('$ck$')
            fft_Exs = np.fft.rfft(Exs[::-1])
            fft_Freq = np.array(range(len(fft_Exs))) * 2*np.pi /(Zs[0]-Zs[-1])
            ax[4].plot(fft_Freq,np.abs(fft_Exs)**2,lw=1,color='b',label='fft($E_x$)')
